...
# 🧠 Feature Engineering (MOVED OUTSIDE OLD BLOCK)
...
col1.metric("Top Gainer", top_gainer['symbol'], f"{top_gainer['percent_change']:.2f}%")
col2.metric("Top Loser", top_loser['symbol'], f"{top_loser['percent_change']:.2f}%")
...
with col1:
    fig = px.bar(df, x='symbol', y='current_price', color='current_price', title="Stock Prices")
    st.plotly_chart(fig, use_container_width=True)

with col2:
    fig2 = px.bar(df, x='symbol', y='percent_change', color='percent_change', title="Performance (%)")
...
fig3 = px.line(filtered_df, y=['high', 'low'], title=f"{selected_stock} High vs Low")
st.plotly_chart(fig3, use_container_width=True)

# 🔥 VOLATILITY
st.subheader("📊 Volatility")

fig4 = px.bar(df, x='symbol', y='volatility', color='volatility')
